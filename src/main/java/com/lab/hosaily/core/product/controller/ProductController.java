package com.lab.hosaily.core.product.controller;
...
@Controller
@RequestMapping(value = "/page/platform/product")
public class ProductController{
    ...
    public ModelAndView index(ModelMap content){
        try{
            return new ModelAndView("platform/product/index", content);
        }catch(Exception e){
    ...
    public ModelAndView list(@PathVariable("organizationId") String organizationId, ModelMap content){
        try{
            ...
            return new ModelAndView("platform/product/list", content);
        }catch(Exception e){
            logger.error(e.getMessage(), e);
            throw new ApplicationException(e.getMessage(), e);
        }
